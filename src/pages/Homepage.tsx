    ...
    <div className="w-full">
      ...
      <div className="md:px-0 px-4 sm:px-6">
            ...
            {data.map((item, index) => (
              <div key={index} className="p-4 bg-white rounded-lg shadow hover:shadow-lg transition duration-300 border-l-4 border-blue-500">
                <p className="text-gray-600 lora-regular-400">
                  <span className="text-sm sm:text-md lora-bold-500 text-gray-800">{item.date}</span> : {item.text}
                </p>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div className="space-y-2">
            <h3 className="font-semibold text-lg sm:text-xl lora-bold-500">Other Links</h3>
